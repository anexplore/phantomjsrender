...
/*
 * This file is copy from selenium-remote-driver-2.53.1
...
public class HttpCommandExecutor implements CommandExecutor, NeedsLocalLogs {
    ...
    /**
     * Creates an {@link HttpCommandExecutor} that supports non-standard {@code additionalCommands}
    ...
    public HttpCommandExecutor(Map<String, CommandInfo> additionalCommands,
            URL addressOfRemoteServer, HttpClientFactory httpClientFactory) {
        try {
            remoteServer =
                    addressOfRemoteServer == null ? new URL(System.getProperty(
        ...
        client = new ApacheHttpClient(httpClientFactory.getHttpClient(), remoteServer);
    ...
    public static HttpClientFactory getDefaultClientFactory() {
        return new HttpClientFactory(10_000, 20_000);
    }

    /**
     * It may be useful to extend the commands understood by this {@code HttpCommandExecutor} at run
     * time, and this can be achieved via this method. Note, this is protected, and expected usage
     * is for subclasses only to call this.
     *
